...
export function renderProtractor(theta, opts = {}) {
  ...
  const baseSide = opts.baseSide || "right";
  ...
  // gekleurde sektor naby die hoekpunt, tussen die twee arms (altyd die klein/binneste boog)
  const sr = 44;
  const aLo = Math.min(armA, armB), aHi = Math.max(armA, armB);
  const sector = `<path d="M ${VX} ${VY} L ${f(px(aLo, sr))} ${f(py(aLo, sr))} A ${sr} ${sr} 0 0 0 ${f(px(aHi, sr))} ${f(py(aHi, sr))} Z" fill="${accent}" fill-opacity="0.18"/>`;

  // arms: vas op arm A, beweeg op arm B — met pyltjies
  const armLen = R + 16;
  ...
  const lblA = `<text x="${f(px(armA, armLen) - 2)}" y="${f(py(armA, armLen)) - 10}" text-anchor="middle" font-family="Fredoka, sans-serif" font-size="15" font-weight="600" fill="${accent}">A</text>`;
  const lblB = `<text x="${f(px(armB, armLen + 9))}" y="${f(py(armB, armLen + 9))}" text-anchor="middle" font-family="Fredoka, sans-serif" font-size="15" font-weight="600" fill="${accent}">B</text>`;
  ...
  const ariaLabel = baseSide === "left"
    ? "Gradeboog met dubbele skaal, arm A aan die linkerkant, en 'n hoek om te meet"
    : "Gradeboog met dubbele skaal en 'n hoek om te meet";

  return `<svg viewBox="0 0 ${VW} ${VH}" width="100%" style="max-width:344px" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="${ariaLabel}">
    ...
    ${ticks}${numsOuter}${numsInner}${sector}${arm(armA)}${arm(armB)}
